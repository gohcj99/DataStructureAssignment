...
public class ProgramArrayList {
	...
	public static void main(String[] args) {
		...
		ArrayList<JobClass> ProcessLists= new ArrayList<JobClass>(); // Create an ArrayList for the Job Queue.
		...
		frame.setTitle("Data Structure Assignment (ArrayList)");
		...
		panel.setLayout(new GridLayout(45,0,1,1));
		...
		SystemClock.scheduleAtFixedRate(
			    	...
			    	int ReleaseTime2;
			    	 ...
			    	 public void run(){   // Run the Program.
			    		 ...
			    		 System.out.println("Time now is: "+TimerCheck.time); // Print out current time.
			    		 		...
			    		 		//int RandomTimer =BurstTimeGenerator.Generate(1,5);
			    		 		
			    		 		//if (TimerCheck.time<12 || (TimerCheck.time)%RandomTimer==0 ){
			    		 		
			    		 		System.out.println ("CHECK: "+ TimerCheck.JobGenerated/((double)TimerCheck.time/60));
			    		 		
			    		 		if (TimerCheck.time<5 || (TimerCheck.JobGenerated/((double)TimerCheck.time/60))<20.0){ // Random Job time Generator when sufficient Job is generated.
			    		 		...
			    		 		if (ProcessLists.isEmpty()==false){ // Only execute with the Queue is not empty.
			    		 		
				    		 		while (x<ProcessLists.size()) { // Loop until all element of the ArrayLists is covered.
						        		
						        		JobClass Test3 = new JobClass(); // Create a class to store Job information.
							        	
							        	Test3 = ProcessLists.get(x); // obtain Job Information from the ArrayLists
							        	
							        	PrintArray = PrintArray +"["+Test3.getjobID()+"="+Test3.getBurstTime()+"("+Test3.getProcessRound()+")] , " ; // Print Queue information.
						        	
							        	System.out.print ("["+Test3.getjobID()+"="+Test3.getBurstTime()+"("+Test3.getProcessRound()+")] "); 
				    			...
				    			label2.setText("Queue("+ProcessLists.size()+") :"+PrintArray); // Display Queue information in JPanel.
			    		 		...
			    		 		if ( Server1Status==1 && (ProcessLists.isEmpty()==false)){ // Only use Server if Server 1 is available and the Queue is not empty
				    		 		...
				    		 		JobClass Print = ProcessLists.get(0); // Obtain Job information from the first element of the ArrayLists.
				    		 		...
				    		 		ProcessLists.remove(0); // Remove the Job from the Queue.
				    		 	...
				    		 	if(JobServer.JobID.getBurstTime()-JobServer.BurstTime==0){ // Execute when the Job have no more burst time.
				    		 		...
				    		 		ProcessLists.add(ProcessLists.size(),JobServer.JobID); // Add back the Job into the Queue.
				    		 	...
				    		 	if ( Server2Status==1 && (ProcessLists.isEmpty()==false) ){
					    		 	...
					    		 	JobClass Print2 = ProcessLists.get(0);
				    		 	...
				    		 	if (TimerCheck.time>=ReleaseTime2 && JobServer2.ServerStatus2==1 ) {
					    		 	...
					    		 	ProcessLists.add(ProcessLists.size(),JobServer2.JobID);
			...
			0,1000); // Execute the entire program loop one timer per one second.
